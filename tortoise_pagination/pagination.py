...
class Pagination(BaseModel):
    ...
    @classmethod
    def from_query(
        cls, offset: NonNegativeInt | None = Query(None), limit: NonNegativeInt | None = Query(10)
    ) -> "Pagination":
        try:
            return cls(offset=offset, limit=limit)
        except ValidationError as error:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=error.errors())
    ...
    async def paginated_response(self, queryset: QuerySet, schema: Type[PydanticModel]) -> Page:
